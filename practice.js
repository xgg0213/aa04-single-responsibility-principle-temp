...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  // 1. Create the combining message
  function recipeF(pieType, recipe) {
    let message = `Combining ingredients for ${pieType}: `
    message += recipe.map(ingredient => ingredient.name).join(', ');
    return message;
  }

  // 2. Print the cost of each pie based on the cost of each ingredient
  function costOfPieF(recipe) {
    return recipe.reduce((prev, current) => {
      return prev + current.cost;
    }, recipe[0].cost);
  }

  costOfPie = costOfPieF(recipe);
  // 3. Calculate the total cost of all the pies
  const totalCost = costOfPie * pieQuantity;

  // 4. Print the total revenue calculated using the given profitMargin
  const revenue = totalCost * (profitMargin || 1.2);

  // 5. print out results
  for (let i = 0; i < pieQuantity; i++) {
    // Print the message
    console.log(recipeF(pieType, recipe));
